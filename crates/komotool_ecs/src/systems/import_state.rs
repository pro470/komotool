...
pub fn import_komorebi_state(
    ...
    for (idx, komo_mon) in state.monitors.elements().iter().enumerate() {
        let mut entity = commands.spawn(Monitor {
            ...
            physical_size: komo_mon.size().clone(),
            work_area_size: komo_mon.work_area_size().clone(),
            work_area_offset: komo_mon.work_area_offset().clone(),
            window_based_work_area_offset: komo_mon.window_based_work_area_offset().clone(),
            window_based_work_area_offset_limit: komo_mon
    ...
    *app_state = AppState {
        ...
        work_area_offset: state.work_area_offset.clone(),
