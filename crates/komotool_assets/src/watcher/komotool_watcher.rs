                    ...
                    for event in events.iter() {
                        match event.kind {
                            ...
                            notify::EventKind::Remove(RemoveKind::Any) => {
                                let (path, is_meta) = get_asset_path(&value, &event.paths[0]);
...
impl FilesystemEventHandler for FileEventHandler {
    ...
    fn get_path(&self, absolute_path: &Path) -> Option<(PathBuf, bool)> {
        println!("get_path({absolute_path:?})");
        let absolute_path = absolute_path.canonicalize().ok()?;
        println!("get_path({absolute_path:?})");
        let (asset_path, is_meta) = get_asset_path(&self.root, &absolute_path);
        println!("asset_path: {asset_path:?}, is_meta: {is_meta} test test test test");
        if asset_path == Path::new("") {
            warn!(
                "FileWatcher::get_path() returned an invalid path: {asset_path:?}, is_meta={is_meta}"
            );
            return None;
        }
    ...
    fn handle(&mut self, _absolute_paths: &[PathBuf], event: AssetSourceEvent) {
        if self.last_event.as_ref() != Some(&event) {
            ...
            if let Err(err) = self.sender.send(event) {
                error!("Failed to send asset event: {err}");
            } else if let Err(err) = self.komotool_sender.send(AppEvent::AssetEvent) {
                error!("Failed to send Komotool asset event: {err}");
            };
        }
    }
}
